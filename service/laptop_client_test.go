...
func TestClientCreateLaptop(t *testing.T) {
	...
	// Asking the server to create a laptop.
	...
	other, err := laptopStore.Find(res.GetId())
	require.NoError(t, err)
...
func TestClientSearchLaptop(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		...
		case 3:
			// 1 << 12 = 2^10 * 4 = 4GB
			laptop.Ram = &pb.Memory{Value: 1 << 12, Unit: pb.Memory_MEGABYTE}
		case 4:
...
func TestClientUploadImage(t *testing.T) {
	...
	testImageFolder := "../tmp"
	laptopStore := stores.NewInMemoryLaptopStore()
	imageStore := stores.NewDiskImageStore(testImageFolder)
	...
	imagePath := fmt.Sprintf("%s/test-400-blows.jpg", testImageFolder)
...
func startTestLaptopServer(t *testing.T, laptopStore stores.LaptopStore, imageStore stores.ImageStore, ratingStore stores.RatingStore) string {
...
func requireSameLaptop(t *testing.T, laptop1 *pb.Laptop, laptop2 *pb.Laptop) {
	json1, err1 := serializer.ProtobufToJSON(laptop1)
	json2, err2 := serializer.ProtobufToJSON(laptop2)
